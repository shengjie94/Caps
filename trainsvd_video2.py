...
class VideoFrameDataset(Dataset):
    def __init__(self, video_dir, frames_per_video=5, width=512, height=320):
...
motion_encoder = MotionEncoder(input_dim=1024, hidden_dim=1024).to(device)
...
optimizer = AdamW(list(unet.parameters()) + list(motion_encoder.parameters()), lr=1e-5)
scaler = GradScaler()
...
def encode_image(pixel_values):
    ...
    motion_embeddings = motion_encoder(image_embeddings)
    
    fused_embeddings = image_embeddings + motion_embeddings
    
    return fused_embeddings

def evaluate_model(dataloader, model, vision_model, vae, device, temporal_loss_weight=0.1):
    model.eval()
    total_loss = 0
    temporal_loss_sum = 0
    with torch.no_grad():
        for inputs in dataloader:
            ...
            temporal_loss = F.mse_loss(denoised_latents[:, :-1], denoised_latents[:, 1:])
            total_loss += (reconstruction_loss.mean() + temporal_loss_weight * temporal_loss).item()
            temporal_loss_sum += temporal_loss.item()

    avg_loss = total_loss / len(dataloader)
    avg_temporal_loss = temporal_loss_sum / len(dataloader)
    print(f"Evaluation | Avg. Loss: {avg_loss:.4f} | Avg. Temporal Loss: {avg_temporal_loss:.4f}")
    model.train()
    return avg_loss, avg_temporal_loss
...
for epoch in range(num_epochs):
    ...
    for i, inputs in enumerate(tqdm(train_dataloader, desc=f"Epoch {epoch + 1}/{num_epochs}", leave=False)):
        ...
        # Temporal Consistency Loss
        temporal_loss = F.mse_loss(denoised_latents[:, :-1], denoised_latents[:, 1:])
        loss = reconstruction_loss + temporal_loss_weight * temporal_loss
    ...
    avg_train_reconstruction_loss = total_reconstruction_loss / len(train_dataloader)
    avg_train_temporal_loss = total_temporal_loss / len(train_dataloader)

    print(
        f"Epoch {epoch + 1}/{num_epochs} | "
        f"Avg. Train Loss: {avg_train_loss:.4f} | "
        f"Reconstruction Loss: {avg_train_reconstruction_loss:.4f} | "
        f"Temporal Loss: {avg_train_temporal_loss:.4f} | "
    ...
    avg_val_loss, avg_val_temporal_loss = evaluate_model(
        val_dataloader, unet, vision_model, vae, device, temporal_loss_weight
    )
    print(
        f"Epoch {epoch + 1}/{num_epochs} | "
        f"Avg. Val Loss: {avg_val_loss:.4f} | "
        f"Avg. Temporal Loss: {avg_val_temporal_loss:.4f}"
    )
